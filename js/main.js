	...
	this.sendFileData = function() {
		...
		xhr.onreadystatechange = function() {
			...
			if(xhr.readyState == 4) {
				...
				var fname = parent.file.name;
				var objTo = document.getElementById('container')
				var divtest = document.createElement("div");
				var ps = parent.file.size / 1024 / 1024;
				ps = Number(ps).toFixed(2);
				divtest.innerHTML = "<div> <p>File: " + fname + " || Size: " + ps +"MB</p></div>";
				...
				{
					document.getElementById("file_names").value = fname;
				}
				else document.getElementById("file_names").value += "|" + fname;
				if(parent.index < document.getElementById('file').files.length)
				{
					//alert(parent.index);
					parent.file = document.getElementById('file').files[parent.index];
					parent.processFiles();
					parent.index++;
				}
